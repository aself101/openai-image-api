...
import { LOG_LEVELS, QUALITIES, BACKGROUNDS, MODERATIONS, OUTPUT_FORMATS, INPUT_FIDELITIES, oneOf, optString, optInt, stringList, } from './cli-options.js';
 ...
 */
export async function dryRun(job, validate) {
 ...
 */
export async function persistResult(api, response, outputDir, model, operation, baseFilename, parameters, requestedFormat, partialPaths) {
 ...
 */
export function requestStem(prompt, tag) {
 ...
 */
export function partialImageWriter(outputDir, stem, format, sink) {
    return async (event) => {
 ...
 */
export async function runRequest(job) {
 ...
 */
export async function resolveOutputDir(options, model) {
 ...
 */
export function buildJobs(options, ctx) {
    ...
    return prompts.map((prompt) => operation === 'edit'
        ? {
            ...base,
            prompt,
            operation,
 ...
 */
export async function runBatch(jobs, options) {
    const failed = [];
    for (const [i, job] of jobs.entries()) {
        const promptNum = jobs.length > 1 ? ` [${i + 1}/${jobs.length}]` : '';
        logger.info(`\n${'='.repeat(60)}`);
        logger.info(job.operation === 'edit'
