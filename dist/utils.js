...
export async function pollVideoWithProgress(api, videoId, options = {}) {
    ...
    showSpinner = true, signal, } = options;
    const startTime = Date.now();
    let spinner = null;
    if (showSpinner) {
    ...
    try {
        while (true) {
            ...
            if (signal?.aborted) {
                if (spinner)
                    spinner.fail('Video generation cancelled');
            ...
            if (elapsed > timeout) {
                if (spinner)
                    spinner.fail('Video generation timed out');
            ...
            if (spinner && video.progress !== undefined) {
                ...
                const statusText = video.status === 'queued'
                    ? 'Queued'
            ...
            if (video.status === 'completed') {
                if (spinner)
                    spinner.stop('Video generation completed');
                return video;
            }
            // Check if failed
