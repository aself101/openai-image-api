...
export function parseTime(text, flag, now = new Date()) {
    ...
    if (rel)
        return midnight(now) - Number(rel[1]) * 86400;
...
export function buildCostProgram(version) {
    ...
    return new Command()
        ...
        .description('Reconcile organization image usage with organization costs, per UTC day and scope. Needs an ADMIN key (OPENAI_ADMIN_KEY).')
        .version(version)
...
function renderRow(r) {
    ...
    const scope = r.scope === 'organization'
        ? 'org'
...
export function renderAssessment(a) {
    ...
    if (a.rows.length === 0)
        lines.push('(no rows: neither endpoint returned data for this range)');
    lines.push('');
    ...
    lines.push('');
    lines.push('Reading this: "image cost" is only spend the Costs endpoint marks as images (quantity_unit=images or a known line item). ' +
...
export async function runCostCli(argv, version) {
    ...
    try {
        ...
        if (!['DEBUG', 'INFO', 'WARNING', 'ERROR'].includes(logLevel)) {
            throw new Error(`Invalid value "${String(raw.logLevel)}" for --log-level. Valid options: DEBUG, INFO, WARNING, ERROR`);
        }
        ...
        const projectIds = Array.isArray(raw.projectId) ? raw.projectId : [];
        const apiKeyIds = Array.isArray(raw.apiKeyId) ? raw.apiKeyId : [];
        ...
        const admin = new OpenAIAdminAPI({ adminKey, logLevel: logLevel });
        const assessment = await admin.assessImageCosts({ start_time: start, end_time: end }, { project_ids: projectIds.length ? projectIds : undefined, api_key_ids: apiKeyIds.length ? apiKeyIds : undefined });
