 ...
 */
export function preserveAmountText(raw) {
...
export class OpenAIAdminAPI {
    ...
    requestTimeout;
    maxAttempts;
    constructor({ adminKey = null, baseUrl = BASE_URL, logLevel = 'WARNING', requestTimeout = 60_000, maxAttempts = 4, } = {}) {
    ...
    _retryable(error) {
        ...
        if (e?.response?.status !== undefined)
            return RETRYABLE_STATUSES.has(e.response.status);
        ...
        for (let attempt = 1;; attempt++) {
            try {
                const response = await axios.get(url, {
                    ...
                    transformResponse: [
                        (data) => (typeof data === 'string' ? JSON.parse(preserveAmountText(data)) : data),
                    ],
                });
                ...
                if (attempt < this.maxAttempts && this._retryable(error)) {
                    ...
                    this.logger.warn(`Admin API ${endpoint}: attempt ${attempt} failed (${getErrorMessage(error)}); retrying in ${delay}ms`);
     ...
     * });
     */
    async listCompletionsUsage(query) {
        return this._allPages(ADMIN_ENDPOINTS.completionsUsage, this._usageQuery(query, { group_by: query.group_by, batch: query.batch }));
    }
    /**
    ...
    async listImagesUsage(query) {
        return this._allPages(ADMIN_ENDPOINTS.imagesUsage, this._usageQuery(query, { group_by: query.group_by, sizes: query.sizes, sources: query.sources }));
    }
    /**
    ...
    async assessImageCosts(range, filters = {}) {
        if (!Number.isInteger(range.start_time) ||
            ...
            throw new OpenAIImageAPIError('range.start_time and range.end_time must be integer Unix seconds with end after start', { type: 'validation_error' });
        }
        ...
        this.logger.info(`Fetched ${completionPages.length} completions, ${imagePages.length} images and ${costPages.length} costs page(s)`);
