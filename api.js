...
export class OpenAIImageAPI {
   ...
   * @param {string} params.model - Model to use (dall-e-2, dall-e-3, gpt-image-1)
   ...
   * @param {string} params.style - Style (dall-e-3 only: vivid or natural)
   * @param {string} params.background - Background (gpt-image-1 only)
   * @param {string} params.output_format - Output format (gpt-image-1 only)
   * @param {number} params.output_compression - Compression 0-100 (gpt-image-1 only)
   * @param {string} params.response_format - Response format (url or b64_json)
   * @returns {Promise<Object>} Generation response with image data
   */
   ...
   * @param {string|string[]} params.image - Image file path(s)
   * @param {string} params.prompt - Text description of desired edit (required)
   * @param {string} params.model - Model to use (dall-e-2 or gpt-image-1)
   * @param {string} params.mask - Mask image file path (optional)
   * @param {string} params.size - Image size
   * @param {number} params.n - Number of images to generate
   * @param {string} params.input_fidelity - Input fidelity (gpt-image-1 only)
   * @param {string} params.background - Background (gpt-image-1 only)
   * @param {string} params.output_format - Output format (gpt-image-1 only)
   * @param {string} params.response_format - Response format (dall-e-2 only)
   * @returns {Promise<Object>} Edit response with image data
   */
   ...
   * @param {string} params.image - Image file path (required)
   * @param {string} params.model - Model to use (only dall-e-2 supported)
   * @param {number} params.n - Number of variations to generate
   * @param {string} params.size - Image size
   * @param {string} params.response_format - Response format (url or b64_json)
   * @returns {Promise<Object>} Variation response with image data
   */
  ...
  async saveImages(response, outputDir, baseFilename, format = 'png') {
    ...
    for (let i = 0; i < response.data.length; i++) {
      ...
      const filename = response.data.length > 1
        ? `${baseFilename}_${i + 1}.${format}`
        : `${baseFilename}.${format}`;
