...
export class OpenAIVideoAPI {
   ...
   */
  private _buildVideoFormData(params: CreateVideoParams): FormData {
    ...
    if (input_reference) formData.append('input_reference', createReadStream(input_reference));
   ...
   */
  private async _makeRequest<T = VideoObject>(
    method: string,
    endpoint: string,
    ...
    try {
      ...
      const requestConfig = {
        ...
        responseType: options.responseType as 'json' | 'arraybuffer' | undefined,
      };
      ...
      if (options.responseType === 'arraybuffer') {
        return Buffer.from(response.data as unknown as ArrayBuffer) as unknown as T;
      }
   ...
   */
  async createVideo(params: CreateVideoParams): Promise<VideoObject> {
    this._verifyApiKey();
    ...
    if (useMultipart) {
      ...
      this.logger.info(
        `Creating video with ${model} and input reference: "${prompt.substring(0, 50)}..."`
      );
    } else {
      // Build JSON payload
      requestData = {
    ...
    this.logger.debug(
      `Request payload: ${useMultipart ? 'multipart/form-data' : JSON.stringify(requestData)}`
    );

    const response = await this._makeRequest<VideoObject>(
      'POST',
      VIDEO_ENDPOINTS.create,
      null,
      requestData,
      useMultipart
    );
   ...
   */
  async retrieveVideo(videoId: string, options: { signal?: AbortSignal } = {}): Promise<VideoObject> {
    this._verifyApiKey();
    ...
    this.logger.debug(
      `Video ${videoId} status: ${response.status} (${response.progress || 0}% complete)`
    );
    return response;
  }
   ...
   */
  async downloadVideoContent(videoId: string, variant: VideoVariant = 'video'): Promise<Buffer> {
    this._verifyApiKey();
    ...
    const buffer = await this._makeRequest<Buffer>(
      'GET',
      endpoint,
      videoId,
      null,
  ...
  async listVideos(params: ListVideosParams = {}): Promise<ListVideosResponse> {
    ...
    const response = await this._makeRequest<ListVideosResponse>('GET', endpoint);
   ...
   */
  async remixVideo(videoId: string, prompt: string): Promise<VideoObject> {
    this._verifyApiKey();
   ...
   * }
   */
  async waitForVideo(videoId: string, options: PollVideoOptions = {}): Promise<VideoObject> {
    this._verifyApiKey();
