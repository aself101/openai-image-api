...
export const VIDEO_MODEL_CONSTRAINTS: VideoModelConstraintsMap = {
  'sora-2': {
...
export function validateModelParams(
  ...
  const errors: string[] = [];
  const constraints = MODEL_CONSTRAINTS[model as ImageModel];
  ...
  if (params.style && constraints.styles && !constraints.styles.includes(params.style)) {
    errors.push(
      `Invalid style "${params.style}" for ${model}. Valid styles: ${constraints.styles.join(', ')}`
    );
  }
  ...
  if (params.response_format && constraints.responseFormats && !constraints.responseFormats.includes(params.response_format)) {
    errors.push(
      `Invalid response_format "${params.response_format}" for ${model}. Valid formats: ${constraints.responseFormats.join(', ')}`
    );
  }
...
export function getModelConstraints(model: string): ImageModelConstraints | null {
  return MODEL_CONSTRAINTS[model as ImageModel] || null;
}
...
export function validateVideoParams(
  model: string,
  params: Partial<CreateVideoParams> & { variant?: string }
): ValidationResult {
  const errors: string[] = [];
  const constraints = VIDEO_MODEL_CONSTRAINTS[model as VideoModel];
  ...
  // Validate quality
  if ((params as { quality?: string }).quality && !constraints.quality.includes((params as { quality: string }).quality)) {
    errors.push(
      `Invalid quality "${(params as { quality: string }).quality}" for ${model}. Valid options: ${constraints.quality.join(', ')}`
    );
  }

  // Validate variant (for content download)
  if (params.variant && !constraints.variants.includes(params.variant as typeof constraints.variants[number])) {
    errors.push(
 ...
 */
export function getVideoModelConstraints(model: string): VideoModelConstraints | null {
  return VIDEO_MODEL_CONSTRAINTS[model as VideoModel] || null;
}
