...
import type { AssessmentRow, CostAssessment, ModelAssessment } from './cost.js';
...
export const floorToUtcDay = (unix: number): number => Math.floor(unix / DAY) * DAY;
/** Unix seconds of the UTC midnight at or after `unix` */
export const ceilToUtcDay = (unix: number): number => Math.ceil(unix / DAY) * DAY;
...
export function parseTime(text: string, flag: string, now: Date = new Date()): number {
  ...
  if (rel) return midnight(now) - Number(rel[1]) * DAY;
...
export function resolveRange(
  ...
  endText: string,
  now: Date = new Date()
): { start_time: number; end_time: number; snapped: boolean } {
...
export function buildCostProgram(version: string): Command {
  ...
  return new Command()
    ...
    .description(
      'Reconcile organization image-model usage with organization costs, per UTC day, scope and model. Needs an ADMIN key (OPENAI_ADMIN_KEY).'
    )
    .version(version)
    .requiredOption(
      '--start <time>',
      'Range start: Unix seconds, ISO-8601, today, yesterday, or <N>d (snapped down to UTC midnight)'
    )
    .option(
      '--end <time>',
      'Range end, exclusive (snapped up to UTC midnight; default: end of the current UTC day)',
      'now'
    )
...
const money = (v: string | null, currency: string | null): string =>
  v === null ? '-' : `${v}${currency ? ` ${currency}` : ''}`;
const num = (v: number | null): string => (v === null ? '-' : String(v));
/** Averages carry 12 decimals in the JSON; the table shows at most six */
const avg = (v: string | null): string => (v === null ? '-' : v.replace(/^(-?\d+\.\d{2,6})\d*$/, '$1'));
const nonZero = (v: string | null): boolean => v !== null && !/^-?0\.0+$/.test(v);

const scopeLabel = (r: AssessmentRow): string =>
...
function renderModel(m: ModelAssessment): string {
...
export function renderAssessment(a: CostAssessment, now: Date = new Date()): string {
  const lines: string[] = [];
  ...
  for (const r of a.rows) {
    ...
    for (const m of r.models) lines.push(renderModel(m));
  }
  if (a.rows.length === 0) lines.push('(no rows: no endpoint returned data for this range)');
  lines.push('');
  const images = a.totals.image_count === null ? '' : `, ${a.totals.image_count} image(s) from the images endpoint`;
  lines.push(
    `Totals: ${a.totals.image_request_count} image-model request(s), ${a.totals.output_image_tokens} output image tokens${images}`
  );
  ...
  if (a.totals.by_model.length) {
    ...
    for (const m of a.totals.by_model) {
      lines.push(
        `  ${pad(m.family, 28)} ${rpad(String(m.requests), 8)} req  ${rpad(String(m.output_image_tokens), 10)} out img tk  ${money(m.image_cost, m.currency)}`
      );
    }
  }
  ...
  if (lastBucket > Math.floor(now.getTime() / 1000) - DAY) {
    notes.push(
      'The Costs endpoint lags usage; the most recent day may show activity with little or no cost yet. Re-run tomorrow for a settled figure.'
    );
  }
  lines.push(...notes);
  return lines.join('\n');
...
export async function runCostCli(argv: string[], version: string): Promise<number> {
  ...
  try {
    ...
    if (range.snapped) {
      logger.info(
        `Range snapped to UTC days: ${new Date(range.start_time * 1000).toISOString()} → ${new Date(range.end_time * 1000).toISOString()}`
      );
    }
