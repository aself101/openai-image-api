...
// Re-export every public type so `import { OpenAIImageAPI, type ImageModel } from 'openai-image-api'`
// works from the main entry; the `./types` subpath remains available.
...
/**
 * A request body, discriminated so the header builder cannot be handed a JSON
 * object while told it is multipart (or the reverse).
 */
type RequestBody = { kind: 'json'; data: Record<string, unknown> } | { kind: 'multipart'; form: FormData };

/** Inputs an edit request will upload, opened and validated */
 ...
 */
function isImageEvent(value: { type: string }): value is { type: string; b64_json: string } {
  return typeof (value as { b64_json?: unknown }).b64_json === 'string';
}
 ...
 */
function streamErrorMessage(value: { type: string }): string {
  const v = value as { error?: { message?: unknown }; message?: unknown };
...
export class OpenAIImageAPI {
  ...
  private deprecationWarned: Set<string>;
  ...
  private _verifyApiKey(): void {
    if (!this.apiKey) {
      throw new OpenAIImageAPIError(
        'API key not set. Please provide apiKey during initialization ' + 'or set OPENAI_API_KEY environment variable.',
        { type: 'configuration_error' }
      );
    }
  }
   ...
   */
  private _warnIfDeprecated(model: string): void {
    const deprecation = getModelDeprecation(model);
    if (!deprecation || this.deprecationWarned.has(model)) return;
   ...
   */
  private _sanitizeErrorMessage(error: unknown, status: number): string {
   ...
   * @throws Error Always
   */
  private _throwApiError(error: unknown): never {
    ...
    if (response) {
      ...
      if (body?.message) {
        this.logger.debug(
          `API error body: status=${status} type=${body.type ?? '-'} code=${body.code ?? '-'} message=${body.message}`
        );
      }
    ...
    const hint =
      code === 'ECONNABORTED' || /timeout/i.test(getErrorMessage(error))
   ...
   */
  private _headers(body: RequestBody): Record<string, string> {
  ...
  private async _makeRequest(
    ...
    if (!options.rateLimited) await this._rateLimit();
    ...
    try {
      ...
      if (typeof body !== 'object' || body === null || !Array.isArray((body as { data?: unknown }).data)) {
        throw new OpenAIImageAPIError(
          `Unexpected response shape from ${endpoint}: expected an object with a data[] array`,
   ...
   */
  private async *_makeStreamRequest<E extends ImageStreamEvent>(
    endpoint: string,
    body: RequestBody,
    options: { rateLimited?: boolean } = {}
  ): AsyncGenerator<E> {
   ...
   */
  private _buildGeneratePayload(params: StreamImageParams & { stream?: boolean }): Record<string, unknown> {
    const {
      prompt,
      model = DEFAULT_MODEL,
      size,
    ...
    if (output_compression !== undefined) payload.output_compression = output_compression;
    ...
    if (stream) {
      payload.stream = true;
      if (partial_images !== undefined) payload.partial_images = partial_images;
    }
    return payload;
  }
   ...
   */
  private _buildEditForm(params: StreamEditImageParams & { stream?: boolean }, inputs: EditInputs): FormData {
    const {
      prompt,
      model = DEFAULT_MODEL,
      size,
    ...
    if (n !== undefined) formData.append('n', n.toString());
    if (size) formData.append('size', size);
    if (quality) formData.append('quality', quality);
    if (inputs.mask) {
      formData.append('mask', inputs.mask.handle.createReadStream({ autoClose: true }), part(inputs.mask));
    }
    if (input_fidelity) formData.append('input_fidelity', input_fidelity);
    if (background) formData.append('background', background);
    if (output_format) formData.append('output_format', output_format);
    if (output_compression !== undefined) formData.append('output_compression', output_compression.toString());
    if (moderation) formData.append('moderation', moderation);
    if (user) formData.append('user', user);
    if (stream) {
      formData.append('stream', 'true');
      if (partial_images !== undefined) formData.append('partial_images', partial_images.toString());
    }
    return formData;
  }

  /** Throw a client-side rejection in the package's error vocabulary */
  private _reject(message: string, type: 'validation_error' | 'input_error', cause?: unknown): never {
   ...
   */
  private _validate(model: string, params: StreamImageParams | StreamEditImageParams, streaming: boolean): void {
    ...
    if (!validation.valid) {
      const hint =
        validation.errors.length === 1 && validation.errors[0]?.startsWith('Unknown model')
   ...
   */
  private _prepareGenerate(params: StreamImageParams, streaming: boolean = false): ImageModel {
    this._verifyApiKey();
   ...
   */
  private _prepareEdit(params: StreamEditImageParams, streaming: boolean = false): ImageModel {
    this._verifyApiKey();
    const model = params.model ?? DEFAULT_MODEL;
   ...
   */
  private async _openEditInputs(params: StreamEditImageParams, model: ImageModel): Promise<EditInputs> {
    const maxSize = getModelConstraints(model)?.imageMaxSize;
    const paths = Array.isArray(params.image) ? params.image : [params.image];
    const opened: ValidatedImage[] = [];
    try {
  ...
  private async _closeEditInputs(inputs: EditInputs): Promise<void> {
   ...
   */
  private _completedToResponse(event: ImageGenerationCompletedEvent | ImageEditCompletedEvent): ImageResponse {
    return {
  ...
  async validateRequest(
    params: StreamImageParams | StreamEditImageParams,
   ...
   */
  async generateImage(params: GenerateImageParams): Promise<ImageResponse> {
   ...
   * }
   */
  async *streamImage(params: StreamImageParams): AsyncGenerator<ImageGenerationStreamEvent> {
    ...
    yield* this._makeStreamRequest<ImageGenerationStreamEvent>(ENDPOINTS.generate, { kind: 'json', data: payload });
  }
   ...
   */
  async generateImageStream(params: StreamImageParams, handlers: StreamHandlers = {}): Promise<ImageResponse> {
   ...
   * });
   */
  async generateImageEdit(params: EditImageParams): Promise<ImageResponse> {
   ...
   * }
   */
  async *streamImageEdit(params: StreamEditImageParams): AsyncGenerator<ImageEditStreamEvent> {
    ...
    try {
      ...
      yield* this._makeStreamRequest<ImageEditStreamEvent>(
        ENDPOINTS.edit,
        { kind: 'multipart', form },
        { rateLimited: true }
      );
   ...
   * );
   */
  async generateImageEditStream(params: StreamEditImageParams, handlers: StreamHandlers = {}): Promise<ImageResponse> {
