...
export class OpenAIImageAPI {
  ...
  async generateImageEdit(params: EditImageParams): Promise<ImageResponse> {
    ...
    // Validate parameters
    const validation = validateModelParams(model, params as unknown as GenerateImageParams);
    if (!validation.valid) {
      throw new Error(`Parameter validation failed:\n  - ${validation.errors.join('\n  - ')}`);
    }
   ...
   */
  async generateImageVariation(params: VariationImageParams): Promise<ImageResponse> {
    this._verifyApiKey();
  ...
  async saveImages(
    ...
    for (let i = 0; i < response.data.length; i++) {
      ...
      const filename =
        response.data.length > 1 ? `${baseFilename}_${i + 1}.${format}` : `${baseFilename}.${format}`;
