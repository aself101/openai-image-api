 ...
 */
async function handleVideoMode(options: CLIOptions): Promise<void> {
  // Determine video model
  let model: VideoModel = 'sora-2'; // default
  ...
  if (options.remixVideo) {
    ...
    const buffer = await videoApi.downloadVideoContent(
      video.id,
      (options.variant as 'video' | 'thumbnail' | 'spritesheet') || 'video'
    );
  ...
  // Download video content
  const variant = (options.variant as 'video' | 'thumbnail' | 'spritesheet') || 'video';
...
program
  .option(
    '--prompt <text>',
    'Text prompt (can specify multiple for batch)',
  ...
  .option(
    '--image <path>',
    'Input image path (can specify multiple for gpt-image-1)',
    (value: string, previous: string[]) => {
      return previous ? [...previous, value] : [value];
    },
    [] as string[]
  ...
  .option(
    '--quality <quality>',
    'Image quality (auto, low, medium, high for gpt-image-1; standard, hd for dalle-3)'
  )
...
async function main(): Promise<void> {
  try {
    ...
    // Determine model
    let model: ImageModel = 'dall-e-2'; // default
    ...
    const constraints: ImageModelConstraints | null = getModelConstraints(model);
    ...
    if (isGenerate) {
      ...
      for (let i = 0; i < options.prompt.length; i++) {
        ...
        const params = {
          ...
          background: options.background as 'auto' | 'transparent' | 'opaque' | undefined,
          moderation: options.moderation as 'auto' | 'low' | undefined,
          output_format: options.outputFormat as 'png' | 'jpeg' | 'webp' | undefined,
          output_compression: options.outputCompression,
          response_format: options.responseFormat as 'url' | 'b64_json' | undefined,
          user: options.user,
        };
      ...
      const params = {
        ...
        quality: options.quality,
        n: options.n,
        input_fidelity: options.inputFidelity as 'high' | 'low' | undefined,
        background: options.background as 'auto' | 'transparent' | 'opaque' | undefined,
        output_format: options.outputFormat as 'png' | 'jpeg' | 'webp' | undefined,
        output_compression: options.outputCompression,
        response_format: options.responseFormat as 'url' | 'b64_json' | undefined,
      ...
      try {
        ...
        spinner.fail(`Variation failed: ${(error as Error).message}`);
        throw error;
      }
    }
